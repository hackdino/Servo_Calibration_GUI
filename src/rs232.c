 ...
 */
void send_to_hardware(int s_nbr, int ms){
	...
	sprintf(msg_f,"%s%02x",msg,crc);
	printf("%s\n",msg_f);
	
	
	RS232_SendByte(com_device[device], 0x1);

	for(i=0;i<7;i++){
		RS232_SendByte(com_device[device], msg_f[i]);
	}
	
	RS232_SendByte(com_device[device], 0x17);
